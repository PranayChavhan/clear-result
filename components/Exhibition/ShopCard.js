...
const ShopCard = () => {
  ...
  return (
    <View className={`w-screen pr-10 mr-3`}>
            <View className="p-4 rounded-3xl bg-orange-50 h-auto flex flex-col overflow-hidden">
               ...
               <View className="bg-white rounded-md">
                ...
                <View className="bg-blue-200 rounded-b-md">
                    <Text className=" text-[10px] text-gray-500 font-semibold my-2 text-center">Verification</Text>


                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingBottom: 10 }}>
      <View style={{ width: '100%', height: 15, backgroundColor: '#D3D3D3', borderRadius: 10, overflow: 'hidden' }}>
        <View style={{ height: '100%', backgroundColor: '#007AFF', borderRadius: 10, width: `${sliderValue * 33.33}%` }} />
      </View>
      <Slider
        style={{ width: '100%', height: 10, alignSelf: 'center', zIndex: 1, position: 'absolute', top: 0 }} // Adjusted slider height
        minimumValue={0}
        maximumValue={3}
        step={1}
        value={sliderValue}
      ...
      />
      <View style={{  paddingHorizontal: 30, width: '100%', position: 'absolute', top: '50%', transform: [{ translateY: -5 }] }}>
        {stops.map((stop) => (
          <View key={stop} style={{ width: 4, height: 4, backgroundColor: '#FFFFFF', borderRadius: 4, position: 'absolute', bottom: -7, left: `${(stop / 3) * 100}%`, zIndex: 1 }} />
        ))}
      </View>
    </View>
