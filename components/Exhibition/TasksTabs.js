  ...
  return (
    <View>
      ...
      >
        <TabButton
          label="Due Tasks"
          onPress={() => handleTabPress("Due Tasks")}
          active={activeTab === "Due Tasks"}
        />
        <TabButton
          label="All Tasks"
          onPress={() => handleTabPress("All Tasks")}
          active={activeTab === "All Tasks"}
        />
      </View>
      <View style={{ backgroundColor: "white" }}>
        {activeTab === "Due Tasks" && <DueTasks clickDueTask={clickDueTask} />}
  ...
  return (
    <View className="border-[0.2px] border-gray-200 rounded-b-lg shadow-2xl bg-white px-4 pt-4 ">
      ...
      {usersData.map((user, index) => (
        <TouchableOpacity onPress={() => clickDueTask()}>
          <View
            key={index}
            className="flex flex-row items-center justify-between my-2"
          >
            <View className="flex flex-row items-center gap-2">
              <Image source={user.image} className="rounded-full w-10 h-10" />
              <View>
                <View className="flex flex-row items-center gap-1">
                  <Text className="text-[14px] font-bold">{user.name}</Text>
                  <Text className="text-[12px] italic font-semibold">
                    ({user.designation})
                  </Text>
                </View>
                <Text className="text-[10px] text-gray-400">
                  Follow up mode: {user.followUpMode}
                </Text>
              </View>
            </View>
